...
func GetStorageByMountPath(mountPath string) (driver.Driver, error) {
	...
	if !ok {
		return nil, errs.Errorf("no mount path for an storage is: %s", mountPath)
	}
...
func CreateStorage(ctx context.Context, storage model.Storage) (uint, error) {
	...
	if err != nil {
		return 0, errs.WithMessage(err, "failed get driver new")
	}
	storageDriver := driverNew()
	// insert storage to database
	err = db.CreateStorage(&storage)
	if err != nil {
		return storage.ID, errs.WithMessage(err, "failed create storage in database")
	}
	// already has an id
	err = initStorage(ctx, storage, storageDriver)
	go CallStorageHooks("add", storageDriver)
	if err != nil {
		return storage.ID, errs.Wrap(err, "failed init storage but storage is already created")
	}
...
func LoadStorage(ctx context.Context, storage model.Storage) error {
	...
	driverNew, err := GetDriver(driverName)
	if err != nil {
		return errs.WithMessage(err, "failed get driver new")
	}
	storageDriver := driverNew()
...
func initStorage(ctx context.Context, storage model.Storage, storageDriver driver.Driver) (err error) {
	...
	if err != nil {
		...
		err = errs.Wrap(err, "failed init storage")
	} else {
...
func EnableStorage(ctx context.Context, id uint) error {
	...
	if !storage.Disabled {
		return errs.Errorf("this storage have enabled")
	}
	storage.Disabled = false
	err = db.UpdateStorage(storage)
	if err != nil {
		return errs.WithMessage(err, "failed update storage in db")
	}
	err = LoadStorage(ctx, *storage)
	if err != nil {
		return errs.WithMessage(err, "failed load storage")
	}
	return nil
}
...
func DisableStorage(ctx context.Context, id uint) error {
	...
	if storage.Disabled {
		return errs.Errorf("this storage have disabled")
	}
	storageDriver, err := GetStorageByMountPath(storage.MountPath)
	if err != nil {
		return errs.WithMessage(err, "failed get storage driver")
	}
	// drop the storage in the driver
	if err := storageDriver.Drop(ctx); err != nil {
		return errs.Wrap(err, "failed drop storage")
	}
...
func UpdateStorage(ctx context.Context, storage model.Storage) error {
	...
	if err != nil {
		return errs.WithMessage(err, "failed get old storage")
	}
	if oldStorage.Driver != storage.Driver {
		return errs.Errorf("driver cannot be changed")
	}
	storage.Modified = time.Now()
	storage.MountPath = utils.FixAndCleanPath(storage.MountPath)
	err = db.UpdateStorage(&storage)
	if err != nil {
		return errs.WithMessage(err, "failed update storage in database")
	}
	if storage.Disabled {
		return nil
	}
	...
	if err != nil {
		return errs.Wrapf(err, "failed drop storage")
	}
...
func DeleteStorageById(ctx context.Context, id uint) error {
	...
	if err = db.DeleteStorageByID(id); err != nil {
		return errs.WithMessage(err, "failed delete storage in database")
	}
	return nil
}
...
func saveDriverStorage(driver driver.Driver) error {
	...
	str, err := utils.JSONTool.MarshalToString(addition)
	if err != nil {
		return errs.Wrap(err, "error while marshal addition")
	}
