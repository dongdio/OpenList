...
var (
	...
	settingChangingCallbacks = make([]func(), 0)
)
...
func SaveSettingItems(items []model.SettingItem) error {
	for i := range items {
		...
		if ok, err := HandleSettingItemHook(item); ok && err != nil {
			return errs.Wrapf(err, "failed to execute hook on %s", item.Key)
		}
	}
	err := db.SaveSettingItems(items)
	if err != nil {
		return errs.Wrap(err, "failed save setting")
	}
	SettingCacheUpdate()
	return nil
}
...
func SaveSettingItem(item *model.SettingItem) (err error) {
	...
	if err = db.SaveSettingItem(item); err != nil {
		return errs.Wrapf(err, "failed save setting on %s", item.Key)
	}
	SettingCacheUpdate()
	return nil
}
...
func DeleteSettingItemByKey(key string) error {
	...
	if err != nil {
		return errs.Wrap(err, "failed to get settingItem")
	}
	if !old.IsDeprecated() {
		return errs.Errorf("setting [%s] is not deprecated", key)
	}
	SettingCacheUpdate()
