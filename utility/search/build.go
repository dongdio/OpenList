...
func Update(parent string, objs []model.Obj) {
	if instance == nil || !instance.Config().AutoUpdate || !setting.GetBool(consts.AutoUpdateIndex) || Running() {
		return
	}
	...
	for i := range objs {
		...
		if !objs[i].IsDir() {
			...
			err = indexSearch(ctx, parent, objs[i])
			if err != nil {
		...
		err = BuildIndex(ctx,
			[]string{dir},
			conf.SlicesMap[consts.IgnorePaths],
			setting.GetInt(consts.MaxIndexDepth, 20)-strings.Count(dir, "/"), false)
		if err != nil {
