...
func GetFileType(filename string) int {
	...
	if SliceContains(conf.SlicesMap[consts.AudioTypes], ext) {
		return consts.AUDIO
	}
	if SliceContains(conf.SlicesMap[consts.VideoTypes], ext) {
		return consts.VIDEO
	}
	if SliceContains(conf.SlicesMap[consts.ImageTypes], ext) {
		return consts.IMAGE
	}
	if SliceContains(conf.SlicesMap[consts.TextTypes], ext) {
		return consts.TEXT
	}
